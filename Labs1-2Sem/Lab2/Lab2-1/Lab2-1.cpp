...
#define wolrd_height 20
// Высота игрового поля 
#define wolrd_width 20
...
void init_world(point world[][wolrd_height])
{
    random_device rd;
    mt19937 gen(rd());
    ...
    for (i = 0; i < wolrd_width; i++) {
        for (j = 0; j < wolrd_height; j++) {
...
void print_world(point world[][wolrd_height])
{
    unsigned int i, j;
    system("cls");
    for (i = 0; i < wolrd_width; i++) {
...
unsigned int get_live_count(point world[][wolrd_height])
{
...
unsigned int count_live_neighbors(point world[][wolrd_height], unsigned int x, unsigned int y)
{
    unsigned int count = 0;
    unsigned int i;
    signed int nb[8][2];
    ...
    for (i = 0; i < 8; i++) {
        ...
        if (_x >= wolrd_width || _y >= wolrd_height) {
            continue;
        }
...
void next_generation(point world[][wolrd_height], point prev_world[][wolrd_height])
{
    unsigned int i, j;
...
void copy_world(point src[][wolrd_height], point dest[][wolrd_height])
{
    unsigned int i, j;
    for (i = 0; i < wolrd_width; i++) {
        for (j = 0; j < wolrd_height; j++) {
...
int cmp_world(point w1[][wolrd_height], point w2[][wolrd_height])
{
    unsigned int i, j;
    for (i = 0; i < wolrd_width; i++) {
        for (j = 0; j < wolrd_height; j++) {
...
int main()
{
    point world[wolrd_width][wolrd_height];
    point prev_world[wolrd_width][wolrd_height];
